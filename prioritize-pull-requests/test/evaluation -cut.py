...
def generate_accept_data(project_full_name, start_time,end_time):
    ...
    with open(accept_data,'w',newline='',errors='ignore') as f:
      ...
      for line in file.readlines():
        try:
            load_dict = json.loads(line)
            if time.strptime(str(load_dict['Timeline'][1]['Created_At'])[2:-2],"%Y-%m-%dT%H:%M:%SZ") >= start_time:
              continue
            if load_dict['Title']:
...
if __name__ == '__main__':
  ...
  for i in range(0, len(date_list)-1):
    ...
    print('{} generate_accept_data...'.format(date_list[i]))
    generate_accept_data('', start_time, end_time)

    print('{} generate_response_data...'.format(date_list[i]))
    generate_response_data('', start_time, end_time)

    print('{} train_accept_model...'.format(date_list[i]))
    train_accept_model()

    print('{} train_response_model...'.format(date_list[i]))
