...
file = open("filter-raw-data.txt","r")
with open('update_without_k.csv','w',newline='',errors='ignore') as f:
  ...
  for line in file.readlines():
    try:
        ...
        count = count + 1
        #print(load_dict['url'])
        if load_dict['url'].find('kubernetes/kubernetes') != -1 and time.strptime(str(load_dict['Timeline'][1]['Created_At'])[2:-2],"%Y-%m-%dT%H:%M:%SZ") >= time.strptime("2018-01-01T00:00:00Z","%Y-%m-%dT%H:%M:%SZ"):
            continue
        Wait_Time = {}
        #print(count)
        print("{}".format(count))
