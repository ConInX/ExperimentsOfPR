...
def get_map(threshold):
	...
	plt.plot(range(1,32),results_accept_response,'b')  
	plt.xlabel("Top-10")  
	plt.ylabel("value(MAP)")  
	plt.title("MAP-Top-10")
	#plt.legend() 
	plt.savefig('map_top_n.png')
	return np.mean(results_accept_response)
